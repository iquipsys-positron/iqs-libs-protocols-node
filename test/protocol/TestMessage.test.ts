...
suite('TestMessage', ()=> {
    ...
    test('Reading and writing message', (done) => {
        ...
        message.value8 = new Date().getTime();
        message.value9 = 99999999;

        message.stream(writeStream);

        let buffer = writeStream.toBuffer();
        assert.lengthOf(buffer, 36);
        ...
        assert.equal(message.value8, message2.value8);
        assert.equal(message.value9, message2.value9);
